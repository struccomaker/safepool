...
export default function GovernancePage() {
  ...
  async function propose() {
    ...
    await fetch('/api/governance/propose', {
      ...
      body: JSON.stringify({ ...form, pool_id: GLOBAL_POOL_ID, proposed_by: 'guest' }),
    })
  ...
  async function castVote(proposalId: string, vote: string) {
    await fetch('/api/governance/vote', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ proposal_id: proposalId, pool_id: GLOBAL_POOL_ID, member_id: 'guest', vote }),
    })
    await loadProposals()
  }
